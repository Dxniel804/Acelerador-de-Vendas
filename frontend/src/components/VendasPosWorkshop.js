...
const VendasPosWorkshop = () => {
  ...
  if (loading) return <div className="vendas-pos-workshop-loading"><div className="loading-spinner"><i className="bi bi-arrow-repeat animate-spin"></i><span>Carregando...</span></div></div>;
    ...
    <div className="vendas-pos-workshop">
          ...
          <div className="management-section">
            <div className="section-header">
              <div className="section-title">
                <div className="section-icon"><i className="bi bi-check-all"></i></div>
                Minhas Vendas
              </div>
            </div>
              ...
              <table className="data-table">
                ...
                <tbody>
                  {minhasVendas.vendas_validadas.length === 0 &&
                    ...
                    <tr>
                      <td colSpan="5" className="text-center py-6 opacity-50">Nenhuma venda registrada ainda.</td>
                    </tr>
                  ) : (
                    <>
                      {minhasVendas.vendas_validadas.map((venda) => (
                        <tr key={`v-${venda.id}`}>
                          ...
                          <td className="text-green-600 font-semibold">R$ {venda.valor_venda?.toLocaleString()}</td>
                          <td>
                      ...
                      {minhasVendas.aguardando_validacao.map((venda) => (
                        <tr key={`a-${venda.id}`}>
                          <td className="text-xs opacity-70">{getNomeProposta(venda)}</td>
                          <td className="font-bold">{venda.cliente_venda || venda.cliente_nome}</td>
                          <td className="text-orange-600 font-semibold">R$ {venda.valor_venda?.toLocaleString()}</td>
                          <td>
                      ...
                      {(minhasVendas.vendas_rejeitadas || []).map((venda) => (
                        <tr key={`r-${venda.id}`} className="bg-red-50/10">
                          ...
                          <td className="text-red-600 font-semibold">R$ {venda.valor_venda?.toLocaleString()}</td>
                          <td>
        ...
        <div className="space-y-6">
          {!showCadastroForm ? (
            ...
            <Card className="form-card">
              <CardHeader>
                <CardTitle><i className="bi bi-plus-lg mr-2"></i> Registrar Venda</CardTitle>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="venda-form">
                  <div className="space-y-4">
                    <div className="form-group-selection">
                      ...
                      <div className="selection-input-wrapper">
                        <Input
                          ...
                          value={propostaSelecionada ? getNomeProposta(propostaSelecionada) + ` – ${propostaSelecionada.cliente_nome}` : 'Clique na lupa para selecionar...'}
                          onClick={() => setShowPropostaModal(true)}
                        />
                        <button type="button" className="selection-button" onClick={() => setShowPropostaModal(true)}>
                    ...
                    <div>
                      ...
                      <Input type="number" value={formData.valor_venda} onChange={(e) => setFormData({ ...formData, valor_venda: e.target.value })} />
                    </div>
                    <div>
                      <Label>Quantidade Vendida *</Label>
                      <Input type="number" value={formData.quantidade_produtos_venda} onChange={(e) => setFormData({ ...formData, quantidade_produtos_venda: e.target.value })} />
                    </div>
                    <div>
                    ...
                    <div className="form-actions mt-6">
                      ...
                      <Button type="button" variant="outline" className="flex-1 btn-cancelar py-6 h-auto" onClick={() => { setShowCadastroForm(false); setPropostaSelecionada(null); }}>
                        CANCELAR
                      </Button>
                    </div>
                  </div>
      ...
      {showPropostaModal && (
        <div className="modal-overlay">
          <div className="modal">
            <div className="modal-header-orange"><h3><i className="bi bi-search mr-2"></i> Selecionar Proposta</h3></div>
            <div className="modal-content">
              <div className="vendas-list">
                {propostasEquipe.filter(p => !['vendida', 'nao_vendida'].includes(p.status)).map((p) => (
                  <div key={p.id} className="venda-item" onClick={() => selecionarProposta(p)} style={{ cursor: 'pointer' }}>
                    <div className="venda-info">
                      <div className="flex items-center gap-2 mb-1">
                        <h4>{getNomeProposta(p)} – {p.cliente_nome}</h4>
                        {p.status === 'nao_vendida' && <Badge className="bg-red-500 text-white border-0 text-[10px] px-1 h-4">Rejeitada</Badge>}
                        {p.status === 'validada' && <Badge className="bg-green-500 text-white border-0 text-[10px] px-1 h-4">Validada</Badge>}
                      </div>
                      <p className="text-sm">R$ {p.valor_proposta?.toLocaleString()} - {p.vendedor_nome}</p>
                    </div>
                    <i className="bi bi-chevron-right"></i>
                  </div>
                ))}
              </div>
              <Button className="w-full mt-6 btn-cancelar py-4" onClick={() => setShowPropostaModal(false)}>FECHAR</Button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay">
          <div className="modal">
            <div className="modal-header-orange">
              <h3><i className="bi bi-pencil-square mr-2"></i> Corrigir Venda</h3>
            </div>
            <div className="modal-content">
              <div className="rejection-notice">
                ...
                <p className="proposta-info"><strong>Proposta:</strong> {getNomeProposta(vendaParaCorrigir)} – {vendaParaCorrigir.cliente_nome}</p>
                {vendaParaCorrigir.motivo_rejeicao_venda && (
                  <p className="motivo-texto"><strong>Motivo:</strong> {vendaParaCorrigir.motivo_rejeicao_venda}</p>
                )}
              </div>
              ...
              <form onSubmit={handleCorrecaoSubmit} className="venda-form space-y-4 mt-4">
                <div>
                  <Label>Valor do Fechamento *</Label>
                  <Input type="number" value={formCorrecao.valor_venda} onChange={(e) => setFormCorrecao({ ...formCorrecao, valor_venda: e.target.value })} required />
                </div>
                <div>
                  <Label>Quantidade Vendida *</Label>
                  <Input type="number" value={formCorrecao.quantidade_produtos_venda} onChange={(e) => setFormCorrecao({ ...formCorrecao, quantidade_produtos_venda: e.target.value })} required />
                </div>
                <div>
                  <Label>Observações</Label>
                  <Textarea value={formCorrecao.observacoes} onChange={(e) => setFormCorrecao({ ...formCorrecao, observacoes: e.target.value })} rows={2} />
                </div>
                ...
                <div>
                  ...
                  />
                  <p className="text-[10px] opacity-60 mt-1">* Se você selecionou novos bônus, é recomendável enviar uma proposta atualizada.</p>
                </div>
                ...
                <div className="form-actions mt-6">
                  ...
                  <Button type="button" variant="outline" className="flex-1 btn-cancelar h-auto py-5" onClick={() => { setShowCorrecaoForm(false); setVendaParaCorrigir(null); }}>
                    CANCELAR
                  </Button>
                </div>
              </form>
