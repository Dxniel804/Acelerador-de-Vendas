...
const StatusControl = ({ token }) => {
  ...
  const statusOptions = [
    ...
    { value: 'pos_workshop', label: 'Pós-Workshop', color: 'yellow', icon: AlertTriangle, description: 'Marcacao de vendas' },
  ...
  const fetchStatus = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:8000/api/admin/status_sistema/', {
        headers: {
