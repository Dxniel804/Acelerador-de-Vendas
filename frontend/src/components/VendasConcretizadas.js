...
const VendasConcretizadas = () => {
  ...
  const [qtdProdutosVenda, setQtdProdutosVenda] = useState('');
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/equipe/vendas-concretizadas/`, {
        ...
        body: JSON.stringify({
          ...
          quantidade_produtos_venda: qtdProdutosVenda,
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Vendas Rejeitadas */}
        {minhasVendas.vendas_rejeitadas && minhasVendas.vendas_rejeitadas.length > 0 && (
          <Card className="mb-8 border-red-200">
            <CardHeader className="bg-red-50">
              <CardTitle className="flex items-center gap-2 text-red-700">
                <XCircle className="h-5 w-5" />
                Vendas Rejeitadas (Necessitam Correção)
              </CardTitle>
            </CardHeader>
            <CardContent className="pt-6">
              <div className="space-y-4">
                {minhasVendas.vendas_rejeitadas.map((venda) => (
                  <div key={venda.id} className="border border-red-200 rounded-lg p-4 bg-red-50/50">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <h3 className="font-semibold text-red-900">{venda.equipe_nome} – Proposta {venda.numero_proposta_equipe || venda.id}</h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm text-gray-700">
                          <div>
                            <span className="font-medium">Cliente:</span> {venda.cliente_venda || venda.cliente?.nome}
                          </div>
                          <div>
                            <span className="font-medium">Valor Venda:</span> {formatarValor(venda.valor_venda)}
                          </div>
                          <div>
                            <span className="font-medium">Produtos:</span> {venda.quantidade_produtos_venda}
                          </div>
                          <div>
                            <span className="font-medium">Data Rejeição:</span> {formatarData(venda.data_rejeicao_venda)}
                          </div>
                        </div>
                        {venda.motivo_rejeicao_venda && (
                          <div className="mt-3 p-3 bg-red-100 rounded text-sm text-red-800">
                            <span className="font-bold">Motivo da Rejeição:</span> {venda.motivo_rejeicao_venda}
                          </div>
                        )}
                      </div>
                      <div className="ml-4">
                        <Button
                          onClick={() => {
                            setPropostaSelecionada(venda);
                            setValorVenda(venda.valor_venda);
                            setClienteVenda(venda.cliente_venda || venda.cliente?.nome);
                            setQtdProdutosVenda(venda.quantidade_produtos_venda);
                            setObservacoesVenda(venda.observacoes_venda || '');
                        ...
                        {podeMarcarComoVendida(proposta) && (
                          <Button
                            onClick={() => {
                              ...
                              setClienteVenda(proposta.cliente?.nome || '');
                              setQtdProdutosVenda(proposta.quantidade_produtos || '');
                              setObservacoesVenda('');
                              setShowMarcarVenda(true);
                            }}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              <h3 className="text-lg font-semibold mb-4">
                {propostaSelecionada?.status === 'nao_vendida' ? 'Corrigir Venda Rejeitada' : 'Marcar Proposta como Vendida'}
              </h3>
              <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                {!propostaSelecionada?.status === 'nao_vendida' && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Valor da Proposta Original</label>
                ...
                <div>
                  <label htmlFor="clienteVenda" className="block text-sm font-medium text-gray-700">Nome do Cliente na Venda *</label>
                  <Input
                    id="clienteVenda"
                    value={clienteVenda}
                ...
                <div>
                  <label htmlFor="valorVenda" className="block text-sm font-medium text-gray-700">Valor Final da Venda *</label>
                ...
                <div>
                  <label htmlFor="qtdProdutosVenda" className="block text-sm font-medium text-gray-700">Quantidade de Produtos *</label>
                  <Input
                    type="number"
                    id="qtdProdutosVenda"
                    value={qtdProdutosVenda}
                    onChange={(e) => setQtdProdutosVenda(e.target.value)}
                    className="mt-1"
                ...
                <div>
                  <label htmlFor="observacoesVenda" className="block text-sm font-medium text-gray-700">Observações</label>
                  <textarea
                    id="observacoesVenda"
