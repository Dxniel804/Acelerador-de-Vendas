...
class Vendedor(models.Model):
    nome = models.CharField(max_length=100)
    codigo = models.CharField(max_length=20, unique=True)
    regional = models.ForeignKey(Regional, on_delete=models.CASCADE, related_name='vendedores')
...
class Workshop(models.Model):
    ...
    regional = models.ForeignKey(Regional, on_delete=models.CASCADE, related_name='workshops')
...
class PerfilAcesso(models.Model):
    ...
    regional = models.ForeignKey(Regional, on_delete=models.SET_NULL, null=True, blank=True, help_text="Regional permitida para acesso de nível gestor")
    ...
    # Permissões Gestor
    def pode_ver_todas_regionais(self):
        return self.nivel in ['administrador', 'geral', 'gerente']
    ...
    def pode_ver_equipes_regional(self):
        """Gestor vê equipes apenas da sua regional"""
        if self.nivel == 'administrador':
            return True
        if self.nivel == 'gestor' and self.regional:
            return True
        return False
    
    def pode_ver_todas_propostas(self):
        """Gestor pode ver propostas da sua regional"""
        if self.nivel == 'administrador':
            return True
    ...
    def pode_ver_regional(self, regional_id):
        if self.nivel in ['administrador', 'geral', 'banca']:
            return True
        if self.nivel == 'gestor' and self.regional and self.regional.id == regional_id:
            return True
        return False
    ...
    def pode_ver_vendedor(self, vendedor_id):
        if self.nivel in ['administrador', 'banca']:
            return True
        if self.nivel == 'gestor' and self.regional:
            return Vendedor.objects.filter(id=vendedor_id, regional=self.regional).exists()
...
class RegraBonus(models.Model):
    """Regras de pontos bônus personalizadas pela banca"""
    nome = models.CharField(max_length=100)
    descricao = models.TextField()
    ...
    def __str__(self):
        return f"{self.nome} (+{self.pontos} pts bônus)"
...
class ConfiguracaoPontuacao(models.Model):
    ...
    def get_configuracao(cls):
        config, created = cls.objects.get_or_create(
            id=1,
            defaults={
                'pontos_proposta_validada': 10,
                'pontos_por_produto': 2
            }
        )
        return config
