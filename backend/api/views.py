...
def login_view(request):
    ...
    print(f"DEBUG: Início da view login_view")
    ...
    if not username or not password:
        print(f"DEBUG: Usuário ou senha vazios")
        return Response({'error': 'Usuário e senha são obrigatórios'}, status=400)
    ...
    if user:
        print(f"DEBUG: Usuário autenticado, buscando perfil")
        try:
            perfil = user.perfil_acesso
            if not perfil.ativo:
                print(f"DEBUG: Perfil inativo")
                return Response({'error': 'Perfil de acesso inativo'}, status=403)
            
            print(f"DEBUG: Perfil OK, criando token")
            token, created = Token.objects.get_or_create(user=user)
            
            print(f"DEBUG: Token criado, retornando resposta")
            return Response({
                'token': token.key,
                'user': {
                    'id': user.id,
            ...
            print(f"DEBUG: PerfilAcesso não existe")
            return Response({'error': 'Perfil de acesso não encontrado'}, status=403)
    else:
...
<<<<<<< HEAD
        ...
        if Equipe.objects.filter(codigo=codigo).exists():
        ...
        if serializer.is_valid():
            ...
            # Criar usuário automaticamente para a equipe com o mesmo nome/código
            from django.contrib.auth.models import User
            from api.serializers import UserSerializer, PerfilAcessoSerializer
            
            user_data = {
                'username': equipe.codigo.lower().replace(' ', '_'),
                'password': equipe.codigo.lower().replace(' ', '_'),  # Senha = nome da equipe
                'email': f'{equipe.codigo.lower().replace(" ", "_")}@equipe.com'
            }
            ...
            if user_serializer.is_valid():
                ...
                # Criar perfil de acesso para o usuário da equipe
                perfil_data = {
                    'usuario': user.id,
                    'nivel': 'equipe',
                    'equipe': equipe.id,
                    'regional': equipe.regional.id if equipe.regional else None
                }
