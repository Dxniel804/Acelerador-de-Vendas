    ...
    try:
        ...
        return Response({
            ...
            'debug': str(e)
                ...
                print(f"DEBUG dashboard_equipe: ERRO ao associar equipe: {str(e)}")
                import traceback
                traceback.print_exc()
                return Response({
                    'error': 'Erro ao associar equipe',
                    'message': str(e)
                }, status=500)
        ...
        # Soma pontos de todas as propostas validadas/vendidas
        pontos_totais = propostas.filter(
            status__in=['validada', 'vendida']
        ).aggregate(
            total=Coalesce(Sum('pontos'), 0)
        ...
        print(f"DEBUG dashboard_equipe: Preparando resposta - Equipe: {perfil.equipe.nome}, Propostas: {propostas.count()}")
        ...
        return Response({
            ...
            # Vendas concretizadas (validada=True)
            'vendas_concretizadas': vendas_validadas,
            # Contadores específicos para os cards do dashboard pós-workshop
            'vendas_validadas': vendas_validadas,
            'vendas_rejeitadas': propostas.filter(status='nao_vendida').count(),
            'pontos_totais': int(pontos_totais),  # NOVO: pontuação total
            'status_sistema': StatusSistema.get_status_atual(),
            'pode_enviar_propostas': perfil.pode_enviar_propostas(),
            'pode_marcar_vendas': perfil.pode_marcar_vendas()
        })
    except AttributeError as e:
        logger.exception('Erro de atributo em dashboard_equipe')
        print(f"DEBUG dashboard_equipe: ERRO AttributeError: {str(e)}")
        import traceback
        traceback.print_exc()
        return Response({
            'error': 'Erro ao acessar dados do perfil',
            'message': str(e),
            'username': request.user.username if hasattr(request, 'user') else 'N/A'
        }, status=400)
    except Exception as e:
        logger.exception('Erro em dashboard_equipe')
        print(f"DEBUG dashboard_equipe: ERRO Exception: {str(e)}")
        import traceback
        traceback.print_exc()
        return Response({
            'error': 'Erro interno do servidor',
            'message': str(e),
            'username': request.user.username if hasattr(request, 'user') and request.user.is_authenticated else 'N/A',
...
def validar_venda_gestor(request, venda_id):
    ...
    try:
        ...
        if perfil.nivel not in ['administrador', 'gestor']:
            return Response({'error': 'Acesso negado'}, status=403)
